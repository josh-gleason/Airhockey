...
int obj_parse_mtl_file(const char *filename, List *material_list)
{
	...
	while( fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream) )
	{
		...
		else if( strequal(current_token, "newmtl"))
		{
			...
			// get the name
			strcpy(current_mtl->name, strtok(NULL, WHITESPACE));
...
/*		
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		else if( strequal(current_token, "mtllib") ) // mtllib
		{
         ...
         // add parent path to front of material filename so that material files can be found
         // when loaded from non-local directories (added by Josh Gleason)
         boost::filesystem::path material_filename(growable_data->material_filename);
         boost::filesystem::path obj_filename(filename);
         material_filename = obj_filename.parent_path() / material_filename;
         strncpy(growable_data->material_filename, material_filename.string().c_str(), OBJ_FILENAME_LENGTH);
